oh-my-posh --init --shell pwsh --config "C:\Users\buste\AppData\Local\Programs\oh-my-posh\themes\davis.omp.json" | Invoke-Expression
...
function cal() {
	python -c "import calendar, datetime; print(calendar.calendar(datetime.datetime.now().year))"
}

function car() { cargo run }
function cab() { cargo build }
function carr() { cargo run --release }
function cabr() { cargo build --release }
